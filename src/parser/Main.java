...
public class Main {
    public static void main(String[] args) throws NullPointerException, ParserConfigurationException, SAXException, IOException {
        ...
        if (Check.check(file_name, "schema.xsd")) { // checking if this XML is valid according to XSD
            ...
            ArrayList<Cake> cakes = dom_parser.parse();
            // Ideally, I would create classes DOMParser and CakeParser, where CakeParser would inherit from the 1st one

            CakeShop cakeShop = new CakeShop(cakes);
            cakeShop.showСakesData();

            System.out.println("\nTotal cakes cost: " + cakeShop.countTotalCost()); // idk why i did it and don't ask me
        }
        else {
